...
function buildLabInstructions(baseInstructions: string, gradeLevel: string, language: string) {
  return [
    ...
    'For visual model requests, prefer exactly one purpose-built tool first: number_line, fraction_strip, decimal_grid, array_model, long_division, integer_chips, ratio_table, data_display, bar_model, place_value_chart, factor_tree, angle_diagram, equation_balance, graph_function, plot_points_on_plane, geometry_figure, table_of_values, or solve_linear_on_canvas. Do not layer write_on_canvas or canvas_action on top unless the student explicitly asks for extra written notes or extra labels on the board.',
