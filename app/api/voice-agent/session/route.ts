...
function buildLabInstructions(baseInstructions: string, gradeLevel: string, language: string) {
  return [
    ...
    'Use learner_context when the student asks to continue from last time, wants review based on past sessions, or asks what they have been struggling with. Use it quietly for pacing and topic choice, not to quote private history.',
...
export async function POST(request: Request) {
  ...
  const curriculumContext = await getLabTutorCurriculumContextForUser(userId).catch((error) => {
    console.error('[voice-agent/session] curriculum context', error)
    return ''
  })
  const instructions = buildLabInstructions(
