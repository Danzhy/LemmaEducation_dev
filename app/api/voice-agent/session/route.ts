...
function buildLabInstructions(baseInstructions: string, gradeLevel: string, language: string) {
  return [
    ...
    'Your board toolkit is intentionally structured, not arbitrary. Use graph_function for polished function graphs from equations, plot_points_on_plane for plotting and connecting ordered pairs, table_of_values for coordinate tables, slope_triangle for rise-run and slope, number_line for number-line models, fraction_strip for fraction bars, fraction_compare for comparing fractions, decimal_grid for tenths, hundredths, percents, and decimal models, array_model for multiplication and area arrays, long_division for whole-number division setup, integer_chips for positive and negative integer models, ratio_table for ratios and rates, data_display for bar charts and line plots, bar_model for tape diagrams and word-problem setup, place_value_chart for digit value and decimals, factor_tree for factors and prime factorization, area_perimeter_model for rectangles and measurement, angle_diagram for angle reasoning, equation_balance for equality models, geometry_figure for standard diagrams, solve_linear_on_canvas for algebra steps, write_on_canvas for short worked notes, annotate_graph_features for graph follow-up labels or highlights, and canvas_action only for the smallest precise follow-up annotations.',
    ...
    'For visual model requests, prefer exactly one purpose-built tool first: number_line, fraction_strip, fraction_compare, decimal_grid, array_model, long_division, integer_chips, ratio_table, data_display, bar_model, place_value_chart, factor_tree, area_perimeter_model, slope_triangle, angle_diagram, equation_balance, graph_function, plot_points_on_plane, geometry_figure, table_of_values, or solve_linear_on_canvas. Do not layer write_on_canvas or canvas_action on top unless the student explicitly asks for extra written notes or extra labels on the board.',
