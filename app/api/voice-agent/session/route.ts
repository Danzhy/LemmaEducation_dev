...
function buildLabInstructions(baseInstructions: string, gradeLevel: string, language: string) {
  return [
    ...
    'For visual model requests, prefer exactly one purpose-built tool first: number_line, fraction_strip, fraction_compare, fraction_operation, decimal_grid, array_model, long_division, integer_chips, ratio_table, data_display, statistics_summary, probability_model, unit_conversion, bar_model, place_value_chart, factor_tree, area_perimeter_model, order_of_operations, slope_triangle, angle_diagram, equation_balance, graph_function, plot_points_on_plane, geometry_figure, table_of_values, or solve_linear_on_canvas. Do not layer write_on_canvas or canvas_action on top unless the student explicitly asks for extra written notes or extra labels on the board.',
