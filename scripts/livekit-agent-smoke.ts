...
async function main() {
  ...
  const invalidVertexStep = await tools.math_check_step.execute(
    { previousStep: 'vertex of y = x^2 - 4x + 3', nextStep: '(4, 3)' },
    { ctx: {} as never, toolCallId: 'smoke-invalid-vertex-step-check' }
  )
  ...
  const validBarChartTotalStep = await tools.math_check_step.execute(
    { previousStep: 'bar chart data: apples 4, bananas 7, grapes 5; total for apples and grapes', nextStep: '9' },
    { ctx: {} as never, toolCallId: 'smoke-bar-chart-total-step-check' }
  )
  const invalidLinePlotComparisonStep = await tools.math_check_step.execute(
  ...
  if (
    !JSON.stringify(validVertexStep).includes('"verdict":"valid"') ||
    !JSON.stringify(validVertexStep).includes('axis of symmetry')
  ) {
    throw new Error(`math_check_step did not accept a valid vertex claim: ${JSON.stringify(validVertexStep)}`)
  }

  if (
    !JSON.stringify(invalidVertexStep).includes('"verdict":"invalid"') ||
    !JSON.stringify(invalidVertexStep).includes('x = 2') ||
    !JSON.stringify(invalidVertexStep).includes('y = -1')
  ) {
    throw new Error(`math_check_step did not reject an invalid vertex claim: ${JSON.stringify(invalidVertexStep)}`)
  }
  ...
  if (
    !JSON.stringify(validBarChartTotalStep).includes('"verdict":"valid"') ||
    !JSON.stringify(validBarChartTotalStep).includes('adding the labeled chart values')
  ) {
    throw new Error(`math_check_step did not accept a bar-chart total claim: ${JSON.stringify(validBarChartTotalStep)}`)
  }

  if (
    !JSON.stringify(invalidLinePlotComparisonStep).includes('"verdict":"invalid"') ||
