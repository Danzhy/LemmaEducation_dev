...
function main() {
  ...
  assertIncludes('workers/livekit-tutor-agent.ts', 'parseJsonObject(ctx.job.metadata)')
  assertIncludes('workers/livekit-tutor-agent.ts', 'maxToolSteps: 4')
  assertIncludes('workers/livekit-tutor-agent.ts', "userAwayTimeout: metadata.audioMode === 'silent' ? null : 45")
  assertIncludes('workers/livekit-tutor-agent.ts', 'textInputCallback')
  assertIncludes('workers/livekit-tutor-agent.ts', 'inputAudioTranscription')
  assertIncludes('workers/livekit-tutor-agent.ts', 'gpt-realtime-whisper')
  assertIncludes('workers/livekit-tutor-agent.ts', "modalities: metadata.audioMode === 'silent' ? ['text'] : ['text', 'audio']")
  assertIncludes('workers/livekit-tutor-agent.ts', "audioEnabled: metadata.audioMode !== 'silent'")
  assertIncludes('workers/livekit-tutor-agent.ts', 'LIVEKIT_TOPICS.control')
  assertIncludes('workers/livekit-tutor-agent.ts', 'session_ready')
